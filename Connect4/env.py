...
class Connect4(Env):
    ...
    #  Reset environment and agents state
    ...
    #  Input: agents (Agent list) and an action from player 1 (int), partial experience of player 2 (list), network
    #  (DQN), epsilon (float), which agent is learning (String), device type (String) and use or not of deterministic
    #  transition(s) (bool)
    ...
    def step(self, agents, action, p2_exp=[], net=None, epsilon=0.0, learning_agent='', device="cpu", det_transition=False):
        ...
        p2_action = p2.choose_action(self.inverse_board(self.board), net, epsilon, device)
    ...
    #  Update board
    #  Input: agent (Agent) and an action (int)
    #  Output: None
    ...
    def win(self, token, copy_board=None):
