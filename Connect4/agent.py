...
class Agent:
    ...
    #  Input: state(int list list), NN (DQN), epsilon rate (float) and device type (String)
    #  Output: action (int)
    def choose_action(self, state, net=None, epsilon=0.0, device="cpu"):
        #  Illegal actions
    ...
    #  Input: state (int list list), set of illegal actions (int list), NN (DQN) and device type (String)
    #  Output: action (int)
    def policy(self, state, illegal_actions, net, device='cpu'):
        ...
        max_idx_list = sorted(range(len(q_vals_list)), key=lambda k: q_vals_list[k], reverse=True)
