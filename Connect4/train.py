...
#
#  Input: filepaths of logs, models and logfile (String), environment (Connect4), agents (Agent list),
#  NN and target NN (DQN),  experience buffers (Experience list list), learning rate (float), epsilon limit, and it's
#  decay rate (float), type of device (String), timestep where the NN training begins (int), frequency of target NN
#  update (int), batch size (int), discount factor (loss calculation) (float), time limit of training (int), use of
#  double-Q learning extension (boolean), list of saved models (String list list), number of steps before changing the
...
def agentTrain(log_dir, model_dir, filenames, env, agents, nets, tgt_nets, buffers, learning_rate, epsilon_final, epsilon_decay, device, replay_start, sync_target, batch_size, gamma, time_limit, ddqn, models, player_change, swap_steps, latest_model_ratio):
    ...
    for i in tqdm(range(time_limit)):
        ...
        action = player_1.choose_action(state, net1, epsilon, device=device)
...
#  reached. save() can stop the training process. This function called at the end of an episode.
#
#  Input: reward (float), environment (Connect4), file to write data (String), list of cumulative reward and ended time
#  horizon (float list), ended timestep of the episode (int), number of finished episode (int), current timestep of the
#  training (int) and learning agent (String)
#
#  Output: file to write data (String), list of cumulative reward and ended time horizon (float list), ended timestep of
...
#  ended time horizon
#  Input: filenames (String list), data to store in filenames (float list list), data labels (String list)
...
#  Input: queue of NN (String Queue), ratio of the latest model (float), environment (Connect4), type of device (String)
...
#  Input: number of episode (int), policy (DQN), environment (Connect4) and learning agent (String)
