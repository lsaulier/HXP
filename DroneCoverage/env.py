...
class DroneAreaCoverage(Env):
    ...
    #  Set a specific position to a agent
    #  Input: agent (Agent) and specific position (int list)
    #  Output: None
    def set_initPos(self, agent, position):
    ...
    #  Input: agents (Agent list) and their actions (int list), device (String), type of transition function (String)
    ...
    def step(self, agents, actions, device="cpu", move="all"):
        ...
        if not self.windless:
            for i in range(len(new_positions)):
                ...
                if move == "all":
                    if actions[i] == 4 or not (actions[i] - 2 == transition or transition - 2 == actions[i]):
    ...
    #  Set a list of actions (this function is used for render method)
